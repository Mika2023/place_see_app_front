...
class RecommendationsViewModel extends ChangeNotifier {
  ...
  Future<void> toggleFavorite(int index, {bool fromSearch = false, bool fromFilter = false}) async {
    if (index == -1) return;
    final placesList = _getListWithMode(fromSearch, fromFilter);
  ...
  List<PlaceCard> _getListWithMode(bool fromSearch, bool fromFilter) {
    if (fromSearch) return _searchResults;
    if (fromFilter) return _filterResults;
    return _places;
  }
}